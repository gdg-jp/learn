...
本編は Windows、macOS、Cloud Shell のセットアップを 1 つだけ進める想定で約 120 分です。Claat の表示時間には複数環境のセットアップと Extra ステップも含まれます。

> **Warning**: 20分程度時間がある場合は Windows / macOS を推奨します。

### Discord で質問する

詰まったところ、進捗、改善したところは Discord サーバーの `#260521-flutter-workshop` に共有してください。招待リンクは下のボタンから開けます。

<button>
  [Discord サーバーに参加する](https://discord.gg/pMTKHQRg)
</button>
...
`lib/feed_page.dart` の `build` メソッドを次の差分のように変更します。
...
`lib/providers/post_providers.dart` の `postsProvider` だけを次の差分のように変更します。
...
`lib/feed_page.dart` の `posts.when(data: ...)` の中身を次の差分のように変更します。
...
> **Tip:** Firestore にはデータが既に入っているためこのメッセージは表示されませんが、`postsProvider` を `return Stream.value([])` するように変更すると見られます。

### error の UI を整える

`lib/feed_page.dart` の `posts.when(error: ...)` の中身を次の差分のように変更します。
...
> **Tip:** 通常はこのメッセージは表示されませんが、Windows / macOS を選択した方はネットワークを切断して再読み込みするとメッセージが見られます。

### loading の UI を整える

`lib/feed_page.dart` の `posts.when(loading: ...)` の中身を次の差分のように変更します。
...
`lib/widgets/post_card.dart` の `build` メソッドを次の差分のように変更します。
