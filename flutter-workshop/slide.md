...
| 使う環境    | 向いている人                         | 起動方法             |
| ----------- | ------------------------------------ | -------------------- |
| Windows     | 自分の PC に環境を残したい人         | `flutter run -d chrome` |
| macOS       | 自分の Mac に環境を残したい人        | `flutter run -d chrome` |
| Cloud Shell | SDK を入れずにブラウザだけで進めたい人 | `flutter run -d web-server ...` |
...
このハンズオンではテンプレートを編集します

- 通常版テンプレート: [gdsc-osaka/flutter-workshop](https://github.com/gdsc-osaka/flutter-workshop)
- Cloud Shell 版: [gdsc-osaka/flutter-workshop-cloudshell](https://github.com/gdsc-osaka/flutter-workshop-cloudshell)
- 完成版: [gdsc-osaka/flutter-workshop-example](https://github.com/gdsc-osaka/flutter-workshop-example)

詰まったときだけ完成版を参照してください
...
## VS Code から Flutter SDK を入れる

![bg right:55% fit](../img/screenshot_01_vscode_install_flutter_extension.png)

Flutter 公式の Quick start と同じ流れで進めます

1. VS Code の拡張機能を開きます
2. `Flutter` を検索します
3. **Flutter** 拡張機能をインストールします
4. 確認が出たら発行元を信頼して進めます

---

## 発行元を信頼してインストールする

![bg right:55% fit](../img/screenshot_02_vscode_do_you_trust_the_publisher_dart_code.png)

この確認が出た場合は **Trust Publisher & Install** を選びます

- Flutter 拡張機能は Dart Code が提供しています
- ここで止まると SDK の Download まで進めません
- インストール後、Command Palette から SDK 設定へ進みます

---

## Flutter SDK を Download する

![bg right:55% fit](../img/screenshot_03_vscode_flutter_new_project_download_sdk.png)

Command Palette から始めます

1. Windows: `Ctrl + Shift + P`
2. macOS: `Cmd + Shift + P`
3. `Flutter: New Project` を選びます
4. SDK が見つからない場合は **Download SDK** を選びます

---

## SDK の保存先

![bg right:55% fit](../img/screenshot_04_vscode_clone_flutter.png)
...
Windows では `C:\Program Files` のようにスペースを含む場所を避けてください

---

## ローカル: Flutter Web を確認する
...
![bg right:55% fit](../img/screenshot_05_terminal_git_clone_and_flutter_run.png)
...
![bg right:65% fit](../img/screenshot_06_chrome_todo_feed.png)
...
![bg right:55% fit](../img/screenshot_91_cloudshell_trust_repository.png)

ブラウザだけで進めたい人はこちらを使います

- Flutter SDK を自分の PC に入れません
- Cloud Shell Editor でコードを書きます
- Web Preview でアプリを確認します
- 通常版とは別のテンプレートを使います
...
下のリンクを開くと、clone とチュートリアル表示が始まります
...
初回起動では利用規約の確認や環境作成に数分かかることがあります

---

## Cloud Shell: Flutter を確認する

![bg right:55% fit](../img/screenshot_92_cloudshell_setup.png)

Cloud Shell のターミナルで実行します
...
`Got dependencies!` が出れば準備できています

---

## Cloud Shell: Web Preview で起動する

![bg right:55% fit](../img/screenshot_93_cloudshell_flutter_run_to_port_8080_preview.png)

Cloud Shell では Chrome ではなく `web-server` で起動します

```bash
flutter run -d web-server --web-hostname 0.0.0.0 --web-port 8080
```

右上の **Web Preview** から **Preview on port 8080** を開いてください
...
![bg right:60% fit](../img/screenshot_07_chrome_loading_posts.png)
...
![bg right:60% fit](../img/screenshot_08_chrome_post_count_3.png)
...
![w:470](../img/screenshot_09_chrome_empty_posts.png)
...
![w:470](../img/screenshot_10_chrome_posts_error.png)
...
![bg right:58% fit](../img/screenshot_08_chrome_post_count_3.png)
...
![bg right:65% fit](../img/screenshot_11_chrome_image_preview.png)

この時点で確認します

- 黒いプレースホルダーが投稿画像に変わります
- 画像の読み込み中はインジケータが出ます
- 壊れた画像 URL ではアイコンが出ます

---

## 本文を重ねた状態

![bg right:65% fit](../img/screenshot_12_chrome_text_overlay.png)

`Stack` の上に本文を置くと、カードらしい見た目になります
...
![bg right:65% fit](../img/screenshot_13_chrome_like_button.png)

投稿カードの右下に操作を置きます

- `likedPostIdsProvider` で押下済みか判定します
- `postActionsProvider` で Firestore を更新します
- 操作を呼ぶ Provider は `ref.read()` です
...
![bg right:65% fit](../img/screenshot_14_chrome_like_checked.png)
