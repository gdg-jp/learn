...
Google Cloud project ID: your-workshop-project-id
...
> **Tips:** `BaseModel` は Agent の出力形式を固定し、`str | None` は文字列または未入力を許可し、`Field(default_factory=list)` は候補ごとに独立した空リストを作ります。
...
> **Tips:** `model` はこの Agent が使う Gemini model を指定し、`output_schema=TravelRequest` は応答を Pydantic model にそろえ、`mode="single_turn"` は会話履歴に依存しない 1 回完結の実行にします。
...
> **Tips:** `ctx.state` は Workflow 内で後続 node が読む共有 state で、`yield Event(route=...)` は次に進む edge を `clarify` または `default` に分岐させます。
...
> **Tips:** `RequestInput` は Workflow を一時停止してユーザー入力を待ち、`build_reclarify_input` は元の依頼、前回の構造化結果、追加回答を 1 つの入力に戻します。
...
> **Tips:** `Workflow(edges=[...])` は node の実行順をコードで定義し、route map は `route_after_clarification` が返した route 名ごとに次の node を選びます。
...
> **Tips:** `TravelOption` は 1 つの候補、`TravelOptions` は候補リスト、`ResearchReport` は検索後の調査結果を表し、`Field(min_length=3, max_length=5)` は出力件数の制約になります。
...
> **Tips:** `strategist_agent` は `TravelRequest` から候補案だけを作る役割で、`output_schema=TravelOptions` によって候補リストの JSON 形式を安定させます。
...
> **Tips:** `research_agent` は `tools=[google_search]` で検索を担当し、`research_report_formatter` は検索メモを `ResearchReport` に整形するため、検索と構造化を 2 段に分けています。
...
> **Tips:** `ctx.run_node(...)` は Workflow node の中から別 Agent を呼ぶ API で、`async def` にしておくと検索や formatter の待ち時間を自然に扱えます。
...
> **Tips:** `asyncio.gather(*tasks)` は候補ごとの調査を並列に実行し、`collect_research_reports` は返ってきた結果を `option_id` keyed dict に集約します。
...
> **Tips:** `candidate_workflow` は候補生成から検索リサーチまでを 1 つの node として親 Workflow に組み込むための nested Workflow です。
...
specialist agent はまだ使わないため、coordinator と ADK Web だけを起動します。
...
> **Tips:** `to_a2a_app(root_agent, default_port=8101)` は Agent を A2A server として公開し、`mode="chat"` は追加確認や再分析依頼を同じ会話として扱いやすくします。
...
> **Tips:** `risk_agent` も comfort と同じ A2A server 形式ですが、`instruction` の評価軸を休業、混雑、天候、遅延などのリスクに寄せています。
...
`connection refused` が出た場合は、`make run-specialists` のターミナルにエラーが出ていないか確認します。port 8101 から 8103 を別プロセスが使っている場合は、そのプロセスを停止してから再実行します。
...
> **Tips:** `Field(ge=1, le=10)` は score の範囲を検証し、`@model_validator(mode="before")` は Pydantic の通常検証前に古い形式を新しい形式へ変換します。
...
> **Tips:** `RemoteA2aAgent` は別プロセスの Agent Card を読んで呼び出す proxy で、`agent_card=remote_agent_card_url(...)` は環境変数があれば Runtime、なければ localhost を使います。
...
> **Tips:** `AgentTool(comfort_agent)` のように包むと、coordinator Agent は remote specialist を tool 呼び出しとして使えるようになります。
...
> **Tips:** `RankedOption` はユーザーに見せる 1 案の推薦情報で、`CoordinatorRecommendation` は上位 3 案、比較要約、調停理由、表示文を 1 つにまとめます。
...
> **Tips:** `build_evaluation_input` は structured object をそのまま渡さず、specialist と coordinator が読みやすいテキスト入力に整えてから評価 Agent に渡します。
...
> **Tips:** `store_recommendation` は Agent の出力を `model_dump()` して `ctx.state` に保存し、後続の選択画面や再提案で同じ推薦結果を参照できるようにします。
...
> **Tips:** `travel_research_workflow` の後ろに `build_evaluation_input -> evaluation_agent -> store_recommendation` を直列接続することで、調査結果がそろってから評価に進みます。
...
specialist を起動してから coordinator と ADK Web を起動します。
...
> **Tips:** `SelectedOptionContext` は planner に渡す入力の境界で、ユーザーが選んだ候補、対応する調査結果、評価だけを 1 つにまとめます。
...
> **Tips:** `ROUTE_SELECTED` と `ROUTE_REPLAN` は選択後の分岐名で、`STATE_SELECTED_OPTION_ID` は planner がどの候補を使うかを後で読むための state key です。
...
> **Tips:** `route_user_selection` はユーザー入力を番号、`option_id`、タイトルで判定し、再提案なら `ROUTE_REPLAN`、候補選択なら `ROUTE_SELECTED` を返します。
...
> **Tips:** `build_replan_input` は現在の `TravelRequest`、推薦結果、変更希望をまとめ直し、再提案を最初からではなく条件更新として扱います。
...
specialist、coordinator、ADK Web を起動します。
...
> **Tips:** ここでは前ステップで追加した `SelectedOptionContext` を再掲し、planner が全候補ではなく選択済み候補だけを見る設計を確認します。
...
> **Tips:** `planner_agent` は `output_schema` を指定しないため自由な Markdown を返し、`PLANNER_AGENT_MODEL` を分けておくと後から planner だけ model を差し替えられます。
...
> **Tips:** `build_planner_input` は `SelectedOptionContext` から必要項目を抜き出し、見出し付きテキストにして planner が詳細旅程を作りやすい入力にします。
...
> **Tips:** `store_itinerary_markdown` は planner の出力を文字列化して `itinerary_markdown` に保存し、次の illustrator が同じ旅程を参照できるようにします。
...
> **Tips:** `model_validate(...)` は state に保存された dict を Pydantic model に戻し、`next(...)` と `reports[selected_option_id]` で選ばれた候補だけを取り出します。
...
> **Tips:** `IMAGE_PROMPT_FORMAT` は画像生成 prompt の型紙で、縦長比率、文字要素、絵の主題を毎回同じ構造で指定するために使います。
...
> **Tips:** `ILLUSTRATOR_PROMPT_AGENT_MODEL` は旅程から画像 prompt を書く model、`ILLUSTRATOR_AGENT_MODEL` は実際に画像を生成する model として分けています。
...
> **Tips:** `illustrator_prompt_agent` は Markdown 旅程を画像生成向けの英語 prompt に変換し、`IMAGE_PROMPT_FORMAT` を instruction に埋め込んで出力の形を誘導します。
...
> **Tips:** `illustrator_agent` は入力 prompt を画像にするだけの薄い Agent で、旅程情報の整理は前段の prompt writer に寄せます。
...
> **Tips:** `store_itinerary_markdown -> illustrator_prompt_agent -> illustrator_agent` の順に並べると、旅程保存、画像 prompt 作成、画像生成が 1 本の route で続きます。
...
```text
[core]
project = your-workshop-project-id
```
