...
セットアップを始める前に、下のフォームに回答してください。回答が終わったら、このページに戻って Cloud Shell の準備へ進みます。

<button>
  [セットアップフォームに回答する](https://docs.google.com/forms/d/e/1FAIpQLSf9hJzKLLElx1bJJio00_jDS0YFMrX56GW6UDq_CVGcI26DLQ/viewform?usp=header)
</button>

### Google Cloud Shell を開く

下のボタンから Cloud Shell を開きます。リポジトリの clone と Cloud Shell Editor の起動が自動で始まります。最初に `cloudshell_tutorial.md` が開きます。

<button>
  [Cloud Shell で初期コードを開く](https://shell.cloud.google.com/cloudshell/editor?cloudshell_git_repo=https://github.com/gdsc-osaka/create-multi-agent.git&cloudshell_workspace=.&cloudshell_open_in_editor=cloudshell_tutorial.md&cloudshell_tutorial=cloudshell_tutorial.md&show=ide%2Cterminal&ephemeral=true)
</button>

![Cloud Shell でリポジトリを信頼して開く確認画面](img/screen_01_cloudshell_open.png)

リポジトリの確認画面が表示されたら、**リポジトリを信頼する** にチェックを入れて **確認** を押します。

![Cloud Shell の認証情報利用を承認する画面](img/screen_02_cloudshell_confirm.png)

Cloud Shell の承認画面が表示されたら、**承認** を押します。
...
Cloud Shell を開くときに、Google アカウントの 2 段階認証プロセスの設定を求められることがあります。表示された場合は、次の手順で 2 段階認証を有効にしてから、もう一度 Cloud Shell のボタンを開いてください。

1. スマートフォンを手元に用意します。
2. [Google アカウント](https://myaccount.google.com/) を開き、Cloud Shell で使うアカウントでログインします。
3. メニューから **セキュリティ** を開きます。
4. **Google にログインする方法** の中にある **2 段階認証プロセス** を選択します。
5. 画面の案内に従って、Google からのメッセージ、確認コードアプリ、電話番号、パスキーなどの確認方法を設定します。
6. 設定が完了したら、このコードラボに戻って **Cloud Shell で初期コードを開く** をもう一度押します。

> **補足:** 2 段階認証プロセスの詳しい説明は [Google アカウント ヘルプ](https://support.google.com/accounts/answer/185839?hl=ja) を確認してください。学校や組織のアカウントで設定できない場合は、管理者に確認してください。

### setup script を実行する

`scripts/setup.sh` は connpass ID を確認し、Google Cloud project ID に固定値 `create-multi-agent` を設定します。続けて `uv sync --extra dev` で依存関係を入れます。

![Cloud Shell Editor のチュートリアルで setup script を確認する画面](img/screen_03_cloudshell_setup_sh.png)
...
`uv` が入っていない場合は、script がインストールします。`uv sync` が完了すると `.venv` が作成されます。`.env` はこの script の実行後に作成されます。
...
GOOGLE_API_KEY=AIzaSyCSMFnZEDI4gAz5zWwKOJ1m7HhHqBS1jdc
...
このステップでは、A2A が何を標準化し、どのような場面で local sub-agent ではなく A2A agent として分けるべきかを確認します。A2A は「エージェントをツールのように包む」ための仕組みではなく、エージェント同士が互いの実装を隠したまま、能力を発見し、タスクを依頼し、成果物を返すための相互運用プロトコルです。

### A2A とは

A2A (Agent2Agent Protocol) は、AI エージェント同士の通信と協調を標準化するオープンプロトコルです。複雑な agentic system では 1 つのエージェントだけでは足りず、検索、評価、予約、社内データ参照などを専門エージェントに分けて協調させる場面が増えます。A2A は、そのようなエージェント間の発見、通信、タスク委譲、成果物交換を標準化します。
...
詳しくは [What is A2A?](https://a2a-protocol.org/latest/topics/what-is-a2a/)、[Core Concepts](https://a2a-protocol.org/latest/topics/key-concepts/)、[Agent Discovery](https://a2a-protocol.org/latest/topics/agent-discovery/) を参照してください。

![A2A の主要 actors: User、A2A Client、A2A Server](img/step4-a2a-actors.png)

A2A の基本的な登場人物は次の 3 つです。

- User: 依頼やゴールを与える人間、または自動化されたサービスです。
- A2A Client (Client Agent): User の代理として A2A protocol で通信を開始するアプリケーション、サービス、またはエージェントです。
- A2A Server (Remote Agent): HTTP endpoint として A2A protocol を公開するリモートエージェントです。Client から見ると内部のメモリ、ツール、推論過程は opaque な black-box として扱われます。

### Why Use the A2A Protocol

A2A は、複数の専門エージェントが別々のチーム、ベンダー、フレームワーク、デプロイ環境で作られていても、共通の方法で連携できるようにするための protocol です。

#### Problems that A2A Solves

たとえば「海外旅行を計画して」と依頼された AI assistant は、航空券、ホテル、現地ツアー、為替、リスク評価など複数の専門 agent に協力してもらう必要があります。A2A がない場合、次の問題が起きやすくなります。

- Agent exposure: エージェントを単純な tool として包むと、交渉、追加確認、multi-turn のやり取り、長時間処理といった agent 本来の能力を表現しにくくなります。
- Custom integrations: 相手ごとに個別 API、認証、データ形式、task tracking を作る必要があり、接続先が増えるほど保守が難しくなります。
- Slow innovation: 新しい専門 agent を追加するたびに bespoke integration が必要になり、実験や置き換えが遅くなります。
- Scalability issues: agent 数と相互作用が増えるほど、点対点の接続は運用上の負荷になります。
- Security gaps: 個別実装が増えると、認証、認可、通信保護、監査の設計がばらつきます。

A2A は、これらを Agent Card、Message、Task、Artifact、標準の transport / security model に分けて扱うことで、agent 同士がより安全かつ安定して協調できるようにします。

#### A2A Example Scenario

このコードラボの旅行計画 agent では、Coordinator Agent が A2A Client になり、3 つの専門 agent を A2A Server として呼び出します。

- Comfort Agent: 移動負荷、休憩、宿泊快適性を評価します。
- Risk Agent: 天候、混雑、移動遅延、予算超過などのリスクを評価します。
- Experience Agent: 体験価値、季節性、旅程の一貫性を評価します。

Coordinator は各 specialist の Agent Card を取得し、`RemoteA2aAgent` 経由で評価タスクを送ります。各 specialist は独立した port で起動するため、同じ Python process 内の helper ではなく、ネットワーク越しに接続される remote agent として扱います。

ADK の公式ドキュメントでは、カスタマーサービス agent が別サービスの product catalog agent から商品情報を取得する例も紹介されています。A2A を使う場合、product catalog agent は A2A Server として公開され、Agent Card に endpoint、capabilities、skills、入出力形式、認証要件を記述します。カスタマーサービス agent は client-side proxy を通して、商品情報の問い合わせを remote agent への task として送れます。

### Core Benefits of A2A

A2A の主な利点は次のとおりです。

- Secure collaboration: HTTPS と標準的な Web security practice を前提にし、エージェントの内部実装を公開せずに連携できます。
- Interoperability: ベンダー、フレームワーク、言語、組織が異なる agent ecosystem をつなげます。
- Agent autonomy: remote agent は自分の能力、状態管理、実行方法を保ったまま task を受けられます。
- Reduced integration complexity: 個別連携の代わりに、Agent Card、Message、Task、Artifact という共通の契約で接続できます。
- Support for long-running operations: Server-Sent Events (SSE)、polling、push notification により、時間のかかる処理や非同期処理を扱えます。

![A2A と MCP は異なるレイヤーを補完する](img/step4-a2a-mcp-readme.png)

A2A と MCP は競合するものではなく、異なる問題を扱います。MCP は model や agent を data、tool、external resource につなぐための protocol です。一方で A2A は、agent が agent として会話し、追加確認し、delegate し、成果物を返すための protocol です。ツール呼び出しで十分な単発処理には MCP が向き、専門 agent の自律性や multi-turn collaboration を残したい場合には A2A が向きます。
...
内部の責務分割だけが目的なら、A2A は過剰です。たとえば入力データを整形する小さな validator、低レイテンシが必要な内部処理、shared memory に直接アクセスする helper は、local sub-agent や通常の関数・クラスとして実装した方が単純です。

### Core Concepts in A2A

A2A の通信は、主に Agent Card、Message、Part、Task、Artifact という単位で考えます。
...
- Message: client と remote agent の 1 turn のやり取りです。role と `messageId` を持ち、指示、追加質問、回答、状態更新などを伝えます。
- Part: Message や Artifact に含まれる content container です。text、file bytes、URL、structured data などを扱えるため、A2A は text-only の protocol ではありません。
- Task: 長時間処理、multi-turn、human-in-the-loop、非同期更新を追跡するための stateful な作業単位です。状態と task ID により、client は処理の進捗を追跡できます。
- Artifact: task の結果として生成される成果物です。文書、画像、構造化データなど、具体的に取り出せる deliverable を表します。

Interaction mechanism としては、通常の request/response、SSE による streaming、長時間処理や disconnected scenario のための push notification が用意されています。今回のローカル実装では、まず request/response と streaming の基本を扱います。
...
Agent Card には endpoint や skill 情報が含まれるため、公開範囲にも注意が必要です。A2A の Agent Discovery ドキュメントでは、内部向け URL や sensitive skill description を含む Agent Card に対して、次の protection mechanism を検討するよう説明しています。

- Authenticated Agent Cards: sensitive information や詳細版 card は、認証後に取得できる Extended Agent Card として分けます。
- Secure endpoints: `/.well-known/agent-card.json` や registry API に mTLS、network restrictions、OAuth 2.0 などの access control を設定します。
- Registry selective disclosure: registry が client の identity と permission に応じて返す Agent Card を変えます。

静的な secret を Agent Card に埋め込むのではなく、out-of-band の dynamic credentials を使う設計が推奨されています。

### A2A Request Lifecycle

公式ドキュメントでは、A2A request lifecycle を大きく 4 段階で説明しています。

![A2A request lifecycle: discovery、authentication、sendMessage、sendMessageStream](img/step4-a2a-request-lifecycle.png)

1. Agent Discovery: Client は `/.well-known/agent-card.json` などから Agent Card を取得し、remote agent の endpoint、skills、capabilities、authentication requirements を確認します。
2. Authentication: Agent Card の `securitySchemes` を見て、必要に応じて OAuth 2.0 / OpenID Connect などで token を取得します。
3. `sendMessage` API: Client は Agent Card の `url` に対して message を送り、server は message を処理して task response を返します。
4. `sendMessageStream` API: streaming が有効な場合、Client は task submitted、working、artifact update、completed といった incremental event を SSE stream として受け取れます。
...
Cloud Shell では、Cloud Shell Editor の **ウェブでプレビュー** から **ポート 8080 でプレビュー** を選択します。
...
このステップでは、全員が同じ coordinator を完成させたあと、specialist agent の評価品質をチームで改善します。2〜3 人チームの場合は、担当する specialist を分けます。

| 担当 | 改善する agent | 主な評価観点 |
| --- | --- | --- |
| メンバー A | `comfort_agent` | 移動負荷、休憩、宿泊快適性、疲労しにくさ |
| メンバー B | `risk_agent` | 天候、混雑、休業、予約困難、交通遅延 |
| メンバー C | `experience_agent` または新規 agent | 非日常性、食、文化、景色、同行者との相性 |

2 人チームの場合は、1 人が `comfort_agent`、もう 1 人が `risk_agent` を担当し、`experience_agent` は余裕があれば改善します。

### 改善方針を決める

担当する specialist について、次の 3 つを先に決めます。

- 高得点にする条件
- 低得点にする条件
- `concerns` に必ず残す注意点

たとえば `comfort_agent` なら、駅近、直通、送迎、休憩しやすさを高く評価し、徒歩距離、乗換の多さ、バス本数の少なさを低く評価します。

### specialist 用 tool を確認する

template には、specialist agent から使える補助 tool が用意されています。

`agents/specialist_tools.py`

```python
estimate_mobility_load(...)
analyze_budget_fit(...)
extract_trip_quality_signals(...)
```
...
| tool | 使いどころ |
| --- | --- |
| `estimate_mobility_load` | 出発地、目的地、交通手段、アクセスメモから移動負荷を見積もる |
| `analyze_budget_fit` | ユーザー予算と候補の概算費用を比べ、予算余裕を判定する |
| `extract_trip_quality_signals` | 希望、制約、調査メモ、リスクから加点・減点の signal を取り出す |

> **補足:** これらの tool は外部 API を呼ばず、入力された候補情報を決定論的に整理します。最新情報の確認は `research_agent` の `google_search` に任せ、specialist 側では評価の一貫性を高める目的で使います。

### agent に tool を追加する

担当する specialist の `agent.py` に tool を追加します。たとえば `comfort_agent` では次のようにします。

```diff python
 from google.adk import Agent
 from agents._common import to_a2a_app
+from agents.specialist_tools import analyze_budget_fit, estimate_mobility_load
 ...
 root_agent = Agent(
     ...
     instruction=(
         "あなたは comfort_agent です。初回評価では EvaluationReport を返してください。"
+        "評価前に estimate_mobility_load と analyze_budget_fit を使い、"
+        "移動負荷と予算余裕を根拠に score を決めてください。"
         "候補ごとの評価は option_evaluations に option_id, score, "
         "comment, concerns を入れてください。"
     ),
+    tools=[estimate_mobility_load, analyze_budget_fit],
     mode="chat",
 )
```

`risk_agent` は `extract_trip_quality_signals` と `analyze_budget_fit`、`experience_agent` は `extract_trip_quality_signals` を中心に使うと比較しやすくなります。

### 新しい specialist を追加する

余裕があるチームは、新しい specialist を追加します。例:

- `budget_agent`: 予算余裕、追加費用、価格変動リスクを評価する
- `food_agent`: 食体験、地域性、予約しやすさ、同行者との相性を評価する
- `accessibility_agent`: 徒歩距離、段差、乗換、休憩場所を評価する

新規 agent を追加する場合は、次の 3 点をそろえます。
...
coordinator 側では `RemoteA2aAgent` を 1 つ追加し、`evaluation_agent` の instruction に新しい評価軸を反映します。

### 改善結果を確認する

ADK Web で同じ入力を使い、改善前後の順位を比べます。
...
- 上位 3 案の順位が、担当 specialist の観点から説明できる
- `comment` と `concerns` に tool の判定結果が反映されている
- `preferred_option_id` が候補の `option_id` と一致している
- 極端な点数ばかりになっていない
...
このステップでは、自分が改善した specialist を Agent Runtime にデプロイし、他のメンバーの coordinator から `RemoteA2aAgent` で呼び出します。

### specialist をデプロイする

自分の担当 agent を含めて deploy script を実行します。

```bash
./scripts/deploy_all.sh
```

デプロイ後、`.agent-runtime-temp` に保存された deploy log から Runtime A2A URL を確認します。

```bash
grep -R "a2a" .agent-runtime-temp/*.deploy.log
```

URL が見つからない場合は、deploy log の末尾を確認します。

```bash
tail -n 40 .agent-runtime-temp/comfort_agent.deploy.log
```

### 他のメンバーの specialist に切り替える

他のメンバーが共有した Runtime A2A URL を `.env` に設定します。

```bash
COMFORT_A2A_URL=https://...
RISK_A2A_URL=https://...
EXPERIENCE_A2A_URL=https://...
TRAVEL_AGENT_A2A_USE_ADC_AUTH=true
```
...
同じ旅行希望を入力し、自分の local specialist と他メンバーの Runtime specialist で推薦順位がどう変わるか確認します。

> **Warning:** Agent Runtime のリソースは課金対象です。共有された URL を使った確認が終わったら、最後のクリーンアップ手順で自分のリソースを削除してください。

### Runtime が使えない場合

権限や時間の都合で Runtime にデプロイできない場合は、local の A2A URL を使って差し替え確認をします。同じ Cloud Shell 内で起動している場合は、次の URL を共有します。
...
別環境のメンバーと共有する場合は、講師の指示に従って Cloud Shell Web Preview や一時的な tunnel を使います。

## Team Challenge: Illustrator の出力品質を改善する

Duration: 0:25:00

このステップでは、planner が作った旅程 Markdown から、より情報量があり、旅行先らしさが伝わる旅しおり画像を生成します。

### 改善観点を決める

画像を改善するときは、次の観点から 2〜3 個を選びます。
...
`agents/coordinator/recommendation_prompts.py` の `IMAGE_PROMPT_FORMAT` を更新します。文字要素、背景、アイコン、禁止事項を具体化すると、結果が安定しやすくなります。
...
```diff python
 IMAGE_PROMPT_FORMAT = """
 {
   ...
   "text_elements": [
     {
-      "text": "AI時代の仕事術",
+      "text": "<destination short title>",
       "position": "top-left",
       "font_style": "bold readable Japanese display font",
       "size": "large",
       "color": "dark navy"
     }
   ],
...
+      "subject": "recognizable travel scenery based on the selected destination",
       "position": "center",
-      "style": "clean commercial illustration"
+      "style": "polished editorial travel illustration with clear local details"
     }
   ],
   "composition": {
-    "background": "dark blue gradient with subtle tech patterns"
+    "hierarchy": "destination title first, then 2-3 itinerary highlights",
+    "safe_area": "keep all text away from edges by 8%",
+    "avoid": "tiny unreadable text, generic city skyline, unrelated landmarks"
   }
 }
 """
```
...
- 目的地名や旅程の重要語が画像に反映されている
- 文字が読める
- 旅行先と関係のないモチーフが混ざっていない
- 生成画像だけを見ても、どんな旅行か説明できる

画像生成が利用できない環境では、`illustrator_prompt` が `session.state` に保存されていることを確認し、prompt の品質を発表対象にします。
...
最後に、チームごとの改善結果を 2 分で発表します。

### 発表する内容

次の順番で説明します。

1. 改善した specialist または illustrator の狙い
2. 変更した prompt、tool、評価軸
3. 改善前後で順位や画像がどう変わったか
4. うまくいかなかった点と、次に直すならどこか

### 評価観点

発表を聞く側は、次の観点でフィードバックします。

| 観点 | 確認すること |
| --- | --- |
| 評価の一貫性 | 同じ入力で、score と comment の理由が説明できる |
| A2A らしさ | specialist を Runtime URL で差し替えられる |
| state 設計 | `option_id`、`research_reports`、`coordinator_recommendation` が壊れていない |
| 旅行品質 | 推薦がユーザー希望、制約、リスクを反映している |
| 画像品質 | 目的地らしさ、読みやすさ、情報量が改善している |

### ふりかえり

最後に、次の問いをチームで話します。

- specialist を分けたことで、どの評価が改善しやすくなったか
- tool で決定論的に補助した方がよい判断と、LLM に任せた方がよい判断は何か
- 新しい specialist を 1 つ追加するなら、どの責務にするか
...
+
+web-memory:
+	TRAVEL_AGENT_USE_MEMORY=true PYTHONPATH=. $(UV_RUN) adk web agents --port 8080 --memory_service_uri="$(MEMORY_SERVICE_URI)"
